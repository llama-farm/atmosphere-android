...
class ConnectionTrain(
    ...
    private suspend fun probeBle(type: String, address: String) {
        // BLE probing is passive - we just check if we've seen the peer recently
        // Actual BLE discovery is handled by BleTransport
        updateStatus(type) { it.copy(
            state = TransportStatus.TransportState.AVAILABLE,
            latencyMs = 50,  // Estimated - BLE is typically low latency
    ...
    private fun getPriority(type: String): Int = when (type) {
        "ble" -> 100      // BLE is fastest and most private
        "lan", "local" -> 80  // LAN is fast and reliable
        "public" -> 50    // Public might have NAT issues
        "relay" -> 10     // Relay is slowest but most reliable
        else -> 0
    }
